...
- **Technology choice: Pydantic for schema validation.** The schema validation layer uses Pydantic models (`TriageOutput`, `TriageInput`, `TraceRecord`). Pydantic was chosen over alternatives (dataclasses + manual validation, attrs, marshmallow, jsonschema) because it combines JSON parsing, type validation, enum constraints, and typed Python objects in a single `model_validate_json()` call — which maps directly to "parse the model's output and validate it" as a single operation. Pydantic is also the standard in the Python LLM ecosystem (used by Instructor, Outlines, LangChain, FastAPI), so the schema definitions are reusable if the project later integrates any of those tools. The actual schema definitions (what fields, what types, what values) are documented in `docs/architecture.md` (forthcoming) and are self-documenting in the Pydantic model code itself.
