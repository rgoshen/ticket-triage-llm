...
- Branch protection on `main` (required PRs + 2 status checks) blocked the bot's direct push of `CHANGELOG.md` and `pyproject.toml`. `GITHUB_TOKEN` respects branch protection by default. Restructured: the workflow now tags the HEAD directly (tags bypass branch protection), creates the GitHub Release immediately, then opens a follow-up PR via `peter-evans/create-pull-request` for the file updates — going through normal branch protection.

**How those issues were resolved:**

All caught during local development or first CI run. Parser fix verified by running the extraction logic against the full project commit history (20-commit sample). Security fix verified by reviewing all `run:` blocks for any remaining `${{ }}` expressions. Infinite-loop guard verified by tracing the event flow: merge → release workflow → `chore(release):` commit pushed → workflow triggers again but `if` condition skips the job. Branch protection fix restructured the workflow to separate tag/release creation (no PR required) from file updates (PR required).
